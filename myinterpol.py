...
def interpolate_slice(
    ...
    for name, arr in scalars.items():
        ...
        out_scalars[name] = griddata(points, vals, (X, Y, Z), method="linear")

    out_vectors = {}
    for name, comps in vectors.items():
        veccomp_interp = np.stack(
            [
                griddata(points, comp.flatten(), (X, Y, Z), method="linear")
                for comp in comps
            ],
            axis=0,
        )
...
def interpolate_slice_spherical(
    ...
    ```
    """
    xdata, ydata, zdata = rthphi_xyz(r, theta, fi)

    vector_projection = drthph_dxyz_3d_schw if gr else drthph_dxyz_3d

    return interpolate_slice(
        xdata,
        ydata,
        zdata,
        scalars,
        {k: vector_projection(r, theta, fi, v[0], v[1], v[2]) for k, v in vectors.items()},
        X,
        Y,
        Z,
        duv,
...
if __name__ == "__main__":
    ...
    plt.pcolormesh(
        ugrid, vgrid, out_scalars["density"], cmap="viridis", shading="auto"
    )
